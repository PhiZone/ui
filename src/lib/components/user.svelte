...
<div class={`card w-full shadow-lg bg-base-100 ${fixedHeight ? 'h-60' : ''}`}>
  <div class={`card-body py-3 px-4 ${mini ? 'flex-row gap-8 justify-between' : ''} items-center`}>
    ...
    {#if !mini}
      <div class={`px-3 w-full text-left ${fixedHeight ? 'h-3/4' : ''}`}>
        ...
        <p>
          <span class="badge badge-primary badge-outline mr-1">{$t('user.exp')}</span>{user.exp}
        </p>
        {#if user.bio}
          <p class="content bio">
            <span class="badge badge-primary badge-outline mr-1">{$t('user.bio')}</span>{user.bio}
          </p>
        {/if}
      </div>
    {/if}
    {#if showButton}
      ...
      >
        {#if !isFollowing}
          <button class="w-fit btn btn-outline btn-primary glass text-sm" on:click={follow}
