<script lang="ts">
  ...
  const replyTo = async (reply: Reply) => {
    replyText = $t('common.reply_to') + '@' + reply.user.username + $t('common.colon');
  };
...
<div class="modal">
  <div class="modal-box bg-base-100 max-h-[90vh] min-w-[70vw] w-[75vw] max-w-[1800px]">
    ...
    <div class="flex items-center mx-5 mb-3">
      ...
      <button
        class={`ml-3 btn ${
          replyText.length > 0 && token ? 'btn-outline btn-primary' : 'btn-ghost btn-disabled'
        } w-1/12 min-w-fit`}
        on:click={() => {
          sendReply();
            ...
            <li class="max-w-full">
              <div class="flex w-full">
                ...
                >
                  ...
                  >
                    <img src={getCompressedImage(reply.user.avatar)} alt="Avatar" />
                  </div>
                  <p class="ml-2 text-base max-w-fit">{reply.user.username}</p>
                  <span class="ml-1 badge badge-sm font-bold">LV{getUserLevel(reply.user.exp)}</span
                  >
                </div>
                <div
    ...
    <figure class="w-1/6 min-w-fit">
      ...
      >
        <a data-sveltekit-preload-data href={`/users/${comment.user.id}`}>
          ...
          >
            <img class="object-fill" src={getCompressedImage(comment.user.avatar)} alt="Avatar" />
          </div>
        </a>
        <a data-sveltekit-preload-data href={`/users/${comment.user.id}`}>
        ...
        <div class="flex gap-1 aspect-[21/5]">
          <span class="badge badge-sm font-bold">LV{getUserLevel(comment.user.exp)}</span>
          {#if comment.user.tag}
  ...
  <div class="card-body w-5/6 pt-6 pl-6 pb-4 pr-4">
    ...
    <div class="card-actions mt-4 flex justify-between items-center">
      ...
      <div class="flex items-center gap-1">
        ...
        <label
          for={`comment-${comment.id}-replies`}
          class={`btn btn-sm btn-primary ${showReplies ? 'btn-active' : 'btn-outline'} gap-2`}
          on:pointerenter={() => {
            if (!replies) {
              getReplies();
            }
