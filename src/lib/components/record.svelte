<script lang="ts">
  ...
  import { getCompressedImage, getGrade, getLevelColor, parseDateTime } from '$lib/utils';
        ...
        <div class="btn-group btn-group-horizontal w-[272px] justify-end">
          <button
            ...
            on:click={(e) => {
              e.preventDefault();
              goto(
                typeof record.chart === 'object' && typeof record.chart.song === 'object'
          ...
          <button
            class={`btn ${getLevelColor(record.chart.level_type)} btn-xs text-sm no-animation`}
            on:click={(e) => {
              e.preventDefault();
              goto(`/charts/${typeof record.chart === 'object' ? record.chart.id : record.chart}`);
            }}
            on:pointerenter={() => {
              preloadData(
                `/charts/${typeof record.chart === 'object' ? record.chart.id : record.chart}`
              );
            }}
          >
            {record.chart.level}
