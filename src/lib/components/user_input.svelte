...
<div class="card card-side w-full bg-base-100 border border-base-300 shadow-lg">
  <figure class="w-1/6 min-w-fit">
    ...
    >
      <p class="opacity-80">{$t('admin.user_input.type')}</p>
      <p class="text-3xl font-extrabold">
        {$t(`admin.user_input.types.${input.type}`)}
      </p>
    </div>
  </figure>
    ...
    <div class="card-actions mt-4 flex justify-between items-center">
      ...
      <div class="flex justify-end gap-4">
        {#if result && originType !== 'User'}
          <div
            class={deleteTip ? 'tooltip tooltip-open tooltip-error tooltip-bottom' : ''}
            data-tip={deleteTip}
          >
            <button
              class={`btn ${
              ...
              on:click={async () => {
                if (originType && originId) {
                  const resp = await api.DELETE(`${types.get(originType)}/${originId}/`, token);
                  if (resp.status === 204) {
                    deleted = true;
                  } else {
              ...
              <p>
                {deleted ? $t('admin.user_input.deleted') : $t('common.delete')}
              </p>
            </button>
          </div>
        {/if}
