		...
		<div class="modal-box bg-base-100 form-control gap-3">
			...
			<div
				class={newCharterStatus === Status.ERROR && newCharterErr
					...
					: "w-full my-2"}
				data-tip={newCharterStatus === Status.ERROR && newCharterErr
					? newCharterErr
					: ""}
			>
							...
							<div class="flex justify-start items-center my-2">
								...
								{#if content.song}
									<a
										...
										)}] {content.song.id}. {content.song.composer} - {content
											.song.name} ({content.song.edition})</a
									>
								{:else}
									<a
										...
										>[{$t("studio.song_submission")}] {content.song_upload?.id}. {content
											.song_upload?.composer} - {content.song_upload?.name} ({content
											.song_upload?.edition})</a
									>
								{/if}
							</div>
							<div class="flex justify-start items-center my-2">
								<span class="w-32 px-4 place-self-center"
									>{$t("studio.submission.song_type")}</span
								>
							...
							{#if songSwitch}
								<div
									class={status === Status.ERROR && error?.song
										...
										: ""}
									data-tip={status === Status.ERROR && error?.song
										? error.song
										: ""}
									on:pointerenter={() => {
										if (songList === null) {
							...
							{:else}
								<div
									class={status === Status.ERROR && error?.song_upload
										? "tooltip tooltip-open tooltip-right tooltip-error"
										: ""}
									data-tip={status === Status.ERROR && error?.song_upload
										? error.song_upload
										: ""}
								>
									<label class="input-group my-2">
