<script lang="ts">
  ...
  let existingTags = $derived(
    createQuery(
      api.tag.listAll(
        {
          rangeNormalizedName: chart?.tags.map((tag) =>
            tag ? tag.replace(/\s/g, '').toUpperCase() : '',
          ),
        },
        { enabled: (!showTags || $submission.isSuccess) && chart && chart.tags.length > 0 },
      ),
    ),
  );
...
{:else if $submission.isError}
  <Error error={$submission.error} back="/studio/chart-submissions/{id}" />
{:else}
