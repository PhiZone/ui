...
export const load: import('./$types').PageLoad = async ({ url, parent, fetch }) => {
    ...
    const resp = await api.GET(`/chart_uploads/${url.search}${getUserPrivilege(user.type) >= 3 ? `${url.search ? "&" : "?"}query_uploader=1` : `${url.search ? "&" : "?"}uploader=${user.id}`}`, access_token, user, fetch);
    if (!resp.ok) {
