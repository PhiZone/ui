...
export const load: import("./$types").PageLoad = async ({
	...
	const resp = await api.GET(
		`/users/${params.id}/?query_relation=1&extra=1&query_song=1`,
		access_token,
		user,
		fetch
	);
	...
	return {
		...
		recentRecords:
			resp.ok && json.extra.recent_records ? json.extra.recent_records : null,
		bestRecords:
			resp.ok && json.extra.best_records ? json.extra.best_records : null,
