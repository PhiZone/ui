	...
	<div class="modal-box bg-base-100 max-h-[90vh] w-[50vw] max-w-[1800px]">
		...
		<label class="input-group my-2">
			...
			>
				...
				<option value="lowest_difficulty"
					>{$t("chart.lowest_difficulty")}</option
				>
				<option value="highest_difficulty"
					>{$t("chart.highest_difficulty")}</option
				>
				<option value="lowest_note_count"
					>{$t("chart.lowest_note_count")}</option
				>
				<option value="highest_note_count"
...
<div class="pt-32 bg-base-200 page form-control justify-center">
	<div class="px-32 flex justify-between items-center mb-6">
		...
		<label
			for="list-options"
			class="btn btn-secondary text-lg btn-xl btn-outline glass"
