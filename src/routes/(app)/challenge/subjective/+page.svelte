<script lang="ts">
    ...
    $: ({ status, questions, error, access_token } = data);
    ...
    async function handleSubmit() {
        ...
        const resp = await api.POST('answers/', formData, access_token, ContentType.FORM_DATA);
        if (resp.ok) {
            console.log('OK');
        } else {
                ...
                <ul class="px-10">
                    {#each questions as question}
                        <li class="py-4">
                            <Question
                                obj={question}
                                bind:getAnswer={getAnswers[question.q.qid - 1]}
                            />
                        </li>
                    {/each}
                </ul>
