...
<svelte:head>
  <title>{$t('recorder.request')} - {$t('recorder.new_request')} | {$t('common.title')}</title>
</svelte:head>
    ...
    <div class="w-3/4 max-w-6xl min-w-20">
      ...
      <div class="card w-full bg-base-100 shadow-lg">
        <div class="card-body">
          ...
          >
            <div class="flex">
              <span class="w-1/4 px-4 place-self-center">{$t('common.form.audio')}</span>
              <input
                type="file"
                accept=".mp3, .ogg"
              ...
              {:else}
                <span class="place-self-center">{$t('common.form.tips.audio')}</span>
              {/if}
            </div>
            <div class="flex">
              <span class="w-1/4 px-4 place-self-center">{$t('common.form.chart')}</span>
              <input
                type="file"
                accept=".json"
                class={`mb-2 place-self-center file:mr-4 file:py-2 file:border-0 file:btn ${
              ...
              {:else}
                <span class="place-self-center">{$t('recorder.chart_placeholder')}</span>
              {/if}
            </div>
            <div class="flex">
              <span class="w-1/4 px-4 place-self-center">{$t('common.form.illustration')}</span>
              <input
                type="file"
                accept=".jpg, .jpeg, .png"
                class={`mb-2 place-self-center file:mr-4 file:py-2 file:border-0 file:btn ${
              ...
              {#if status === Status.ERROR && error?.illustration}
                <span class="place-self-center text-error">{error.illustration}</span>
              {:else}
                <span class="place-self-center">{$t('recorder.illustration_placeholder')}</span>
              {/if}
            </div>
            {#if !(noteSize || resolution || composer || illustrator || charter || tip || username || rks || challengeDifficulty)}
              <div class="flex">
                ...
                <input
                  ...
                  accept=".set"
                  class={`mb-2 place-self-center file:mr-4 file:py-2 file:border-0 file:btn ${
                    status === Status.ERROR && error?.config
                      ? 'input-error file:btn-error'
                      : 'input-primary file:btn-outline file:bg-primary'
                  }`}
                  on:change={handleConfig}
                />
                {#if status === Status.ERROR && error?.config}
                  <span class="place-self-center text-error">{error.config}</span>
                {:else}
                  <span class="place-self-center">{$t('recorder.config_placeholder')}</span>
                {/if}
                <button
            ...
            {#if !config}
              ...
              <div class="flex my-3 gap-1">
                <span class="w-1/4 px-4 place-self-center">{$t('recorder.music_volume')}</span>
              ...
              <div class="flex my-3 gap-1">
                <span class="w-1/4 px-4 place-self-center">{$t('recorder.hitsound_volume')}</span>
                <p class="text-xl font-bold w-1/12">
              ...
              <div class="flex my-3">
                <span class="w-1/4 px-4 place-self-center">{$t('recorder.loading_option')}</span>
                <input
                  type="checkbox"
            ...
            {/if}
            <div class="flex my-3">
              <span class="w-1/4 px-4 place-self-center">{$t('recorder.ending_option')}</span>
              <input
                type="checkbox"
            ...
            {#if endingOption}
              <div class="flex">
                <span class="w-1/4 px-4 place-self-center">{$t('recorder.avatar')}</span>
                <input
                  type="file"
                  accept=".jpg, .jpeg, .png"
                  class={`mb-2 place-self-center file:mr-4 file:py-2 file:border-0 file:btn ${
                    (!song && dataIncomplete) || (status === Status.ERROR && error?.avatar)
                      ? 'input-error file:btn-error'
                      : 'input-primary file:btn-outline file:bg-primary'
                  }`}
                ...
                {#if status === Status.ERROR && error?.avatar}
                  <span class="place-self-center text-error">{error.avatar}</span>
                {:else}
                  <span class="place-self-center">{$t('recorder.illustration_placeholder')}</span>
                {/if}
              </div>
